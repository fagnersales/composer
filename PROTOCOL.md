# Composer — protocol & API reference

The contract between the board, the hub server, and fleet agents. If
you're a fleet agent serving a board, this file is the source of truth;
the human-facing intro is [README.md](README.md).
...
`images/` in the task folder holds reference images uploaded from the
board's composer, content-addressed as `<sha1-prefix>.<ext>`
(png/jpg/gif/webp). Requests point at them via the `images` field.
